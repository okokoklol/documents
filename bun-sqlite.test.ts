...
function setup() {
  return new BunSQLiteDocumentOps({
...
describe("write + read", () => {
  ...
  test("multiline line count", async () => {
    const ops = setup();
    await ops.write({ path: "a.txt", content: "a\nb\nc" });
    expect((await ops.read({ path: "a.txt" })).lines).toBe(3);
  });
...
describe("glob", () => {
  ...
  test("auto-prepends **/", async () => {
    const ops = setup();
    await ops.write({ path: "deep/nested/file.txt", content: "" });
    const results = await ops.glob({ pattern: "file.txt" });
    expect(results).toContain("deep/nested/file.txt");
  });
});
